...
DiffusionFluxAux::validParams()
{
  ...
  params.addParam<double>("coefficient_multiplier",
                          "Use it when there is need to scale or multiply the flux with some "
                          "coefficient, e.g. (-1) to change fluxes to stresses");
  return params;
}
...
DiffusionFluxAux::DiffusionFluxAux(const InputParameters & parameters)
  : AuxKernel(parameters),
    ...
    _coefficient(getParam<double>("coefficient_multiplier"))
...
DiffusionFluxAux::computeValue()
{
  ...
  return -_coefficient * diffusion_coef * gradient;
}
